...
namespace SOR4_Swapper
{
    public partial class SwapperLevels : Form
    {
        ...
        public SwapperLevels(MainWindow mainwindow)
        {
            InitializeComponent();
        ...
        private void itemList_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if (Library.levelDictionary[e.Index].Path == "n/a")
            {
        ...
        private void cmbItemOriginalList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmb.Text != "") picThumbOrig.Image = _mainwindow.thumbnailslib.getThumbnail("level", cmb.SelectedIndex);
            if (cmb.SelectedIndex != -1)
            {
                if (Library.levelDictionary[cmb.SelectedIndex].Path == "n/a")
                {
                    cmb.SelectedIndex = -1;
                }
            }
        ...
        {
            ComboBox cmb = (ComboBox)sender;
            if (cmb.Text != "") picThumbSwap.Image = _mainwindow.thumbnailslib.getThumbnail("level", cmb.SelectedIndex);
            if (cmb.SelectedIndex != -1)
            {
                if (Library.levelDictionary[cmb.SelectedIndex].Path == "n/a")
                {
